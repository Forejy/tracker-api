...
db.issues.aggregate([
  {
    $group: {
      _id: { owner: '$owner', status: '$status' }, // Fait des sous-groupes en regroupant chaque duo owner/status
      count: { $sum: 1 }, // puis compte le nombre d'éléments de chaque sous-groupe
    },
  },
]);
// Le résultat ce sera une liste  d'objets ave une propriété _id, à laquelle est assignée un autre objet avec 2 propriétés owner et status, puis une autre propriété count.
// Donc, en français, le résultat c'est chaque duo avec le nombre de fois où ce duo est présent dans la liste des documents